...
# Discriminator decision boundaries for each stage
discriminator_boundaries = [
    # Stage 1: Initial - discriminator easily separates
    {"boundary": lambda x, y: 0.8 * x + 0.6 * y - 1.0, "confidence": 0.9},
    # Stage 2: Early learning - boundary starts adapting
    {"boundary": lambda x, y: 0.7 * x + 0.5 * y - 0.8, "confidence": 0.7},
    # Stage 3: Mid learning - more complex boundary
    {"boundary": lambda x, y: 0.6 * x + 0.4 * y - 0.5, "confidence": 0.6},
    # Stage 4: Advanced learning - boundary becomes more sophisticated
    {"boundary": lambda x, y: 0.5 * x + 0.3 * y - 0.3, "confidence": 0.5},
    # Stage 5: Near convergence - very subtle boundary
    {"boundary": lambda x, y: 0.4 * x + 0.2 * y - 0.1, "confidence": 0.4},
    # Stage 6: Final - discriminator struggles (no clear boundary)
    {"boundary": lambda x, y: 0.1 * x + 0.1 * y + 0.1, "confidence": 0.3}
]
...
for i, stage in enumerate(learning_stages):
    ...
    # Add discriminator decision boundary
    disc_boundary = discriminator_boundaries[i]
    Z_boundary = disc_boundary["boundary"](X, Y)
    
    if i == 0:  # Initial generator (top-left) - no background
        ax.contour(X, Y, rv_stage.pdf(pos), colors=stage["color"], alpha=0.8, linewidths=2)
        # Strong discriminator boundary
        ax.contour(X, Y, Z_boundary, levels=[0], colors='black', alpha=0.8, linewidths=2, linestyles='-')
        # Confidence region
        ax.contourf(X, Y, Z_boundary, levels=[-10, 0], colors=['red'], alpha=0.1)
        ax.contourf(X, Y, Z_boundary, levels=[0, 10], colors=['blue'], alpha=0.1)
    elif i == 5:  # Real data (bottom-right) - no background
        ax.contour(X, Y, rv_stage.pdf(pos), colors=stage["color"], alpha=0.9, linewidths=2)
        # Very weak discriminator boundary (nearly random)
        ax.contour(X, Y, Z_boundary, levels=[0], colors='gray', alpha=0.3, linewidths=1, linestyles='--')
        ...
        ax.contour(X, Y, rv_stage.pdf(pos), colors=stage["color"], alpha=0.8, linewidths=2)
        # Discriminator boundary gets weaker over time
        ax.contour(X, Y, Z_boundary, levels=[0], colors='black', alpha=disc_boundary["confidence"], linewidths=2)
        # Confidence regions
        ax.contourf(X, Y, Z_boundary, levels=[-10, 0], colors=['red'], alpha=0.05)
        ax.contourf(X, Y, Z_boundary, levels=[0, 10], colors=['blue'], alpha=0.05)
...
# Add legend for discriminator boundaries
from matplotlib.lines import Line2D
legend_elements = [
    Line2D([0], [0], color='black', lw=2, label='Discriminator Boundary'),
    Line2D([0], [0], color='red', lw=0, marker='s', markersize=10, alpha=0.3, label='Fake Region'),
    Line2D([0], [0], color='blue', lw=0, marker='s', markersize=10, alpha=0.3, label='Real Region')
]
...
plt.savefig('images/gan_learning_progression.png', dpi=150, bbox_inches='tight', 
            facecolor='white', edgecolor='none')
plt.close()
```

![GAN Learning Progression](images/gan_learning_progression.png)
...
plt.tight_layout()
plt.savefig('images/gan_training_dynamics.png', dpi=150, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.close()
```

![GAN Training Dynamics](images/gan_training_dynamics.png)
...
plt.tight_layout()
plt.savefig('images/gan_applications.png', dpi=150, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.close()
```

![GAN Applications](images/gan_applications.png)
...
plt.tight_layout()
plt.savefig('images/mode_collapse_demo.png', dpi=150, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.close()
```

![Mode Collapse Demonstration](images/mode_collapse_demo.png)
:::
...
::: {.incremental}
- **Extension of vanilla GAN** with conditional information
- Both Generator and Discriminator receive **additional input** (labels, attributes, etc.)
- Enables **controlled generation** of specific outputs
- First introduced by Mirza & Osindero (2014)
:::
...
- Input: Random noise $z$ + condition $y$
- Output: Fake sample $x_{fake}$
- Learns: $P(x|y)$ distribution

**Discriminator**: $D(x, y)$

- Input: Sample $x$ + condition $y$
- Output: Probability of being real
- Learns to distinguish real/fake given condition
:::

## How Training & Generation Works

::: {.incremental}
**During Training:**

- Real data comes with labels: $(x_{real}, y_{real})$
- Example: (image of cat, label = "cat"), (image of dog, label = "dog")
- Condition $y$ is **given from the dataset**
- Generator learns: "When $y$ = cat, generate cat images"

**During Generation (Inference):**

- You **manually specify** the condition $y$
- Want a cat? Set $y$ = "cat" + random noise $z$ → $G(z, y)$ generates cat
- Want a dog? Set $y$ = "dog" + random noise $z$ → $G(z, y)$ generates dog
- **Full control** over what to generate!
:::

## Conditional GAN Objective

Modified objective function with conditioning:

$$\min_G \max_D V(D,G) = \mathbb{E}_{x \sim p_{data}(x)}[\log D(x|y)] + \mathbb{E}_{z \sim p_z(z)}[\log(1 - D(G(z|y)))]$$

Where $y$ represents the conditional information (class labels, attributes, text descriptions, etc.)

## Applications of cGAN

::: {.incremental}
1. **Text-to-Image**: Generate images from text descriptions
2. **Class-Conditional Generation**: Generate specific object classes (e.g., MNIST digits 0-9)
3. **Image-to-Image Translation**: Pix2Pix - edges to photos, sketches to realistic images
4. **Super-Resolution**: Generate high-res images conditioned on low-res input
5. **Style Transfer**: Transfer artistic style while preserving content
6. **Data Augmentation**: Generate labeled synthetic data for training
:::

## Example: MNIST Conditional Generation
...
# Simulate conditional generation - different digit classes
fig, axes = plt.subplots(2, 5, figsize=(12, 5))
fig.suptitle('Conditional GAN: Generating Specific Digit Classes', fontsize=14, color='black')

# Simulate 10 different digit classes (0-9)
np.random.seed(42)
for idx in range(10):
    row = idx // 5
    col = idx % 5
    ...
    # Create a simple synthetic "digit" pattern for each class
    digit_image = np.zeros((28, 28))
    
    # Create different patterns for different digits
    if idx == 0:  # Circle for 0
        y, x = np.ogrid[-14:14, -14:14]
        mask = (x**2 + y**2 <= 100) & (x**2 + y**2 >= 64)
        digit_image[mask] = 1
    elif idx == 1:  # Vertical line for 1
        digit_image[5:23, 12:16] = 1
    elif idx == 2:  # Wavy pattern for 2
        for i in range(28):
            j = int(14 + 8 * np.sin(i * 0.3))
            if 0 <= j < 28:
                digit_image[i, max(0, j-1):min(28, j+2)] = 1
    else:  # Random patterns for others
        digit_image = np.random.rand(28, 28) > 0.7
        digit_image = digit_image.astype(float)
    
    # Add noise
    digit_image += np.random.randn(28, 28) * 0.1
    digit_image = np.clip(digit_image, 0, 1)
    
    ax.imshow(digit_image, cmap='gray')
    ax.set_title(f'Class: {idx}', fontsize=10, color='black')
    ax.axis('off')

plt.tight_layout()
plt.savefig('images/conditional_gan_demo.png', dpi=150, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.close()
```

![Conditional GAN Example](images/conditional_gan_demo.png)

## Key Advantages

::: {.incremental}
- ✅ **Control**: Generate samples with specific attributes
- ✅ **Flexibility**: Condition on various types of information
- ✅ **Quality**: Often better quality than vanilla GAN
- ✅ **Interpretability**: Easier to understand what's being generated
- ✅ **Practical**: More applicable to real-world tasks
:::
...
plt.tight_layout()
plt.savefig('images/performance_metrics.png', dpi=150, bbox_inches='tight',
            facecolor='white', edgecolor='none')
plt.close()
```
